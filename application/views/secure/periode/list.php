<!-- <div class="content-body pt-4 mt-4"> -->
	...
	<div class="container-fluid pt-4 mt-4">
									...
									<tr align="center">
										...
										<td>
											...
											<div class="btn-group mb-3 btn-group-sm" role="group">
												<a data-toggle="modal"
													data-target="#modaledit_<?php echo $this->enkripsi->encrypt_url($periode->PeriodesId) ?>"
													href="<?php echo base_url('secure/periode/edit/' . $this->enkripsi->encrypt_url($periode->PeriodesId)) ?>"
													class="btn btn-icon btn-info"><i class="far fa-edit" data-toggle="tooltip"
														data-placement="top" title="Ubah"></i></a>
											</div>
											<?php else : ?>
											<div class="btn-group mb-3 btn-group-sm" role="group">
												<a href="<?php echo base_url('secure/periode/aktid/' . $this->enkripsi->encrypt_url($periode->PeriodesId)); ?>"
													class="btn btn-primary"
													data-confirm="Perhatian ! | Apakah Kamu yakin Ingin Mengaktifkan Periode Ini ?"
													data-confirm-yes="window.location.href='<?php echo base_url('secure/periode/aktif/' . $this->enkripsi->encrypt_url($periode->PeriodesId)); ?>' ">
		...
		<!-- Modal Add-->
		<form action="<?php echo base_url('secure/periode/tambah'); ?>" enctype="multipart/form-data" method="post"
					...
					<div class="modal-content">
						...
						<div class="modal-body">
							<div class="form-group">
								<input type="hidden" name="koordinator"
									value="<?php echo $this->enkripsi->encrypt_url($koordinator['DosenId']) ?>">
								<input type="hidden" name="kaprodi"
									value="<?php echo $this->enkripsi->encrypt_url($kaprodi['DosenId']) ?>">
								...
								<div class="row">
									...
									<div class="col-6">
										...
										<input name="t_akademik" type="text" min="2020" max="3000" class="form-control" maxlength="9"
											placeholder="Tahun">
									</div>
								</div>
	...
	<!-- Modal Edit-->
	<form action="<?php echo base_url('secure/periode/edit'); ?>" enctype="multipart/form-data" method="post"
		accept-charset="utf-8" class="form-horizontal">
		<div class="modal fade" id="modaledit_<?php echo $this->enkripsi->encrypt_url($periode['PeriodesId']) ?>"
			...
			<div class="modal-dialog" role="document">
              ...
              ?>
							<option value="<?php echo $this->enkripsi->encrypt_url($periode['PeriodesKoordinator']); ?>" selected>
								<?php echo $koor['DosenNama'] ?></option>
							<?php $dosen = $this->db->query("SELECT * FROM tdosen WHERE DosenNidn LIKE '__________' ORDER BY DosenNama ASC")->result_Array(); ?>
							<?php foreach ($dosen as $dosen) { ?>
							<option value="<?php echo $this->enkripsi->encrypt_url($dosen['DosenId']); ?>">
								<?php echo $dosen['DosenNama']; ?>
							</option>
							<?php } ?>
						</select>
              ...
              ?>
							<option value="<?php echo $this->enkripsi->encrypt_url($periode['PeriodesKaprodi']); ?>" selected>
								<?php echo $kapr['DosenNama'] ?></option>
							<?php $dosen = $this->db->query("SELECT * FROM tdosen WHERE DosenNidn LIKE '__________' ORDER BY DosenNama ASC")->result_Array(); ?>
							<?php foreach ($dosen as $dosen) { ?>
							<option value="<?php echo $this->enkripsi->encrypt_url($dosen['DosenId']); ?>">
								<?php echo $dosen['DosenNama']; ?>
						...
						<div class="row">
							<div class="col-6">
								...
								<select name="semester" class="form-control">
									<option value="<?php echo $periode['PeriodesSemester'] ?>" class="form-control daterange-cus"
										selected>
                    ...
                    ?>
							...
							<div class="col-6">
								<br>
								<label>Tahun Akademik</label>
								<input value="<?php echo $periode['PeriodesTahunakademik'] ?>" name="t_akademik" type="text"
									class="form-control" maxlength="9" placeholder="Tahun">
							</div>
						</div>
						...
						<div class="row">
							<div class="col-6">
								<br>
								<label>Periode Selesai</label>
								<input value="<?php echo $periode['PeriodesSelesai'] ?>" name="p_selesai" type="date"
